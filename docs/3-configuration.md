...
* **cgroups**: create a cgroup-v2 in */sys/fs/cgroup/sandbox-uid* and attach every children fedid to the corresponding cgroup (binder need to be privileged to activate cgroups)
...
* **label**: set SeLinux/Smack security label for fedid children to either: LSM_LABEL_SBOX, LSM_LABEL_API, LSM_LABEL_CMD or LSM_LABEL_NO. Tagging children with a security label requirer either SMACK or SeLinux on the platform as well as running under privileged mode.
...
* api://fedid/ping // assert binder is alive
* api://fedid/info // return parsed config to automatically build HTML5 debug/test page [binder-devtool]({% chapter_link monitoring-doc.binder-devtool %})

*Note: In following samples 'fedid' should be replaced by what ever you chose as API name in your config.json.*
...
Children fedid task output always come back as events. The model depend on chosen encoder. Default one returns stdout+stderr with closing status at the end of children execution.

```json
{
  "jtype": "afb-event",
  "event": "fedid/sandbox-simple/display@22131",
  "data": {
    "cmd": "display",
    "pid": 22131,
    "status": {
